...
const landing = () => {
  ...
  return (
    <div className='h-[100vh]  w-full min-h-screen  relative overflow-hidden flex justify-center text-white'>
      ...
      <div className='lg:w-[60vw] w-[90vw] h-[60%] flex flex-col justify-center items-center z-30  mt-[50px] gap-2 bg-blue-900/10 rounded-lg backdrop-blur-xs border-2 border-white/20 shadow-lg shadow-black/30'>
        ...
        <div className='flex gap-10 justify-center items-center text-3xl mt-8   font-serif'>
            <button onMouseEnter={()=> setcolorbutton(true)} onMouseLeave={()=> setcolorbutton(false)} 
            onClick={()=> scrollToSection('contact')} className={` py-5 px-7 rounded-3xl ${ colorbutton ? 'bg-[#371b26]' : 'bg-[#1b2637]'
            } hover:cursor-pointer transition-all ease-out`}>Join Us</button>
            <button onMouseEnter={()=> setcolorbutton1(true)} onMouseLeave={()=> setcolorbutton1(false)} onClick={()=> scrollToSection('about')} className={` py-5 px-7 rounded-3xl ${ colorbutton1 ? 'bg-[#1b2337]' : 'bg-[#372c1b]'
